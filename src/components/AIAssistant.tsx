...
export default function AIAssistant() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#08090c] p-8 text-gray-200 flex flex-col justify-between h-[85vh] scroll-smooth">
      {/* Header */}
      ...
      {/* Conversations log */}
      <div className="flex-1 bg-white/[0.02] border border-white/5 rounded-2xl p-6 overflow-y-auto flex flex-col gap-4 mb-6 pr-2">
        {messages.map((m) => {
            ...
            >
              <div
                className={`p-4 rounded-2xl text-xs leading-relaxed font-sans ${
                  isAssistant
                    ? 'bg-purple-950/20 border border-purple-500/10 text-gray-200'
                    : 'bg-gradient-to-tr from-purple-600 to-indigo-600 text-white shadow-lg shadow-purple-500/10'
                }`}
              >
                <div className="whitespace-pre-wrap">{m.text}</div>
              </div>
              <span className="text-[9px] text-gray-500 mt-1.5 px-1 font-mono uppercase">{m.timestamp}</span>
            </div>
          );
        })}
        ...
        {aiLoading && (
          <div className="self-start text-left max-w-[80%] flex flex-col">
            <div className="p-4 rounded-2xl bg-purple-950/25 border border-purple-500/10 text-gray-300 text-xs font-sans flex items-center gap-2">
      ...
      {/* Templates Row */}
      <div className="mb-4">
        <span className="text-[10px] text-gray-500 font-bold uppercase font-mono tracking-wider block mb-2 text-left">Quick Query Templates</span>
        <div className="flex flex-wrap gap-2 justify-start">
          {quickTemplates.map((t) => (
            <button
              ...
              disabled={aiLoading}
              className="text-[11px] bg-white/5 border border-white/10 hover:border-purple-500/40 hover:bg-purple-500/10 text-gray-300 font-semibold px-3 py-2 rounded-xl cursor-pointer transition-all disabled:opacity-50 font-sans"
            >
              {t.label}
            </button>
          ))}
      ...
      >
        <input
          ...
          disabled={aiLoading}
          className="w-full bg-white/5 text-sm text-gray-200 px-5 py-4 rounded-xl border border-white/10 focus:outline-none focus:ring-1 focus:ring-purple-500 transition-all font-sans pr-14"
        />
